...
/** `company.settings.liveOrderNotifications` — LeafLink order arrival toasts. Default on. */
export function extractLiveOrderNotificationsEnabled(config: unknown): boolean {
  const o = config && typeof config === "object" ? (config as Record<string, unknown>) : null;
  const company = o?.company && typeof o.company === "object" ? (o.company as Record<string, unknown>) : null;
  const settings =
    company?.settings && typeof company.settings === "object"
      ? (company.settings as Record<string, unknown>)
      : null;
  if (!settings || settings.liveOrderNotifications === undefined) return true;
  return settings.liveOrderNotifications !== false;
}
