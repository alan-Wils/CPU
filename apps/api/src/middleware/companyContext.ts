...
function allowsMissingCompany(req: Request): boolean {
    ...
    if (method === "POST" && /\/nexbatch\/staff\/invite$/.test(path))
        return true;
    if (method === "GET" && /\/nexbatch\/staff$/.test(path))
        return true;
    if (method === "PATCH" && /\/nexbatch\/staff\/[^/]+$/.test(path))
        return true;
    if (method === "DELETE" && /\/nexbatch\/staff\/invites\/[^/]+$/.test(path))
        return true;
    ...
    if (method === "POST" && /\/admin\/usage-costs\/sync$/.test(path))
        return true;
    if (method === "GET" && /\/admin\/usage-costs$/.test(path))
        return true;
    if (method === "POST" && /\/admin\/usage-costs\/manual-override$/.test(path))
        return true;
