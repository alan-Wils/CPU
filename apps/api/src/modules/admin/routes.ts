...
adminRouter.post(
  "/users/:userId/status",
  requireRole(["OWNER", "ADMIN"]),
  validate({ body: adminUserStatusSchema }),
  asyncHandler(async (req, res) => {
...
adminRouter.patch(
  "/users/:userId",
  requireRole(["OWNER", "ADMIN"]),
  validate({ body: adminUserUpdateSchema }),
  asyncHandler(async (req, res) => {
    const payload = req.body as { email?: string; role?: string; isActive?: boolean };
...
adminRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const payload = req.body as { email: string; role: any };
