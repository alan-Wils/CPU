/** Server copy of lib/sourceBatchActive.ts — keep in sync for store reconciliation. */
...
function parseAmountToLbs(value: unknown): number {
...
export function getSourceOriginalLbs(source: unknown): number {
    ...
    if (s.weightLbs !== undefined)
        return num(s.weightLbs);
    if (s.grams !== undefined)
        return num(s.grams) / 453.592;
    if (s.amount !== undefined)
        return parseAmountToLbs(s.amount);
    return 0;
}
