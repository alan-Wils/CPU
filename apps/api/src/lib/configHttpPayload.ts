...
export function buildIntegrationsMetaView(merged: MergedCompanyConfig): Record<string, unknown> {
    ...
    return {
        ...
        metrcLastStrainsSyncAt:
            String(metrc.metrcLastStrainsSyncAt ?? metrc.metrcSandboxLastStrainsSyncAt ?? "").trim() ||
            null,
        lastStrainsSync:
            String(metrc.metrcLastStrainsSyncAt ?? metrc.metrcSandboxLastStrainsSyncAt ?? "").trim() ||
            null,
        ...
        metrcLastPackagesSyncAt:
            String(metrc.metrcLastPackagesSyncAt ?? metrc.metrcSandboxLastPackagesSyncAt ?? "").trim() ||
            null,
        lastPackagesSync:
            String(metrc.metrcLastPackagesSyncAt ?? metrc.metrcSandboxLastPackagesSyncAt ?? "").trim() ||
            null,
