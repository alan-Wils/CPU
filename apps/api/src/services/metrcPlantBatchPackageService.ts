...
import { buildMetrcPlantBatchPackageBody } from "../lib/metrcPlantBatchPackageBodies.js";
...
const PLANT_BATCH_PACKAGE_ENDPOINT = "/plantbatches/v2/packages";
...
export type MetrcPlantBatchPackageRequestDebug = {
...
export type MetrcPlantBatchPackageInput = {
  companyId: string;
  actorUserId: string;
...
export type MetrcPlantBatchPackageSuccess = {
  ok: true;
  status: number;
  message: string;
  endpoint: string;
  requestPayload: { pathname: string; body: unknown; requestDebug: MetrcPlantBatchPackageRequestDebug };
  requestDebug: MetrcPlantBatchPackageRequestDebug;
...
export type MetrcPlantBatchPackageFailure = {
  ok: false;
  status: number;
  message: string;
...
export type MetrcPlantBatchPackageResponse =
  | MetrcPlantBatchPackageSuccess
  | MetrcPlantBatchPackageFailure;
...
function buildRequestDebug(input: {
  ...
  baseUrl: string;
  payloadBody: unknown;
}): MetrcPlantBatchPackageRequestDebug {
  return {
    licenseNumber: input.licenseNumber,
    authMode: input.authMode,
    baseUrl: input.baseUrl,
    endpoint: PLANT_BATCH_PACKAGE_ENDPOINT,
    payloadBody: input.payloadBody,
  };
}

export class MetrcPlantBatchPackageService {
  async createPackageFromPlantBatch(
    input: MetrcPlantBatchPackageInput,
  ): Promise<MetrcPlantBatchPackageResponse> {
    logInfo("[METRC] plant_batch_package_start", {
    ...
    if (loaded.environment !== "sandbox") {
      return {
        ...
        status: 403,
        message:
          "Create package from plant batch is sandbox-only. Switch METRC environment to sandbox.",
      };
    }
    ...
    if (!plantBatchName) {
      return {
        ok: false,
        status: 400,
        message: "Plant batch name is required for METRC plant batch package creation.",
      };
    }
    ...
    if (!itemName) {
      return {
        ok: false,
        status: 400,
        message: "METRC item name is required for plant batch package creation.",
      };
    }
    ...
    if (isMetrcSandboxPlaceholderLicense(license)) {
      const locationsRequest = await resolveMetrcLocationsActiveRequest({
        ...
        loaded,
        companyId: input.companyId,
        purpose: "plant_batch_package_test",
      });
    ...
    const requestBody = buildMetrcPlantBatchPackageBody({
      plantBatchName,
      packageTag: input.packageTag,
      count: input.count,
    ...
    const pathname = `${PLANT_BATCH_PACKAGE_ENDPOINT}${licenseQuery(license)}`;
    const endpointKey = PLANT_BATCH_PACKAGE_ENDPOINT;
    ...
    if (isMetrcClientFailure(result)) {
      ...
      await appendMetrcPlantBatchRequestLog({
        companyId: input.companyId,
        action: "plant_batch_package_test",
        method: "POST",
      ...
      logWarn("[METRC] plant_batch_package_failed", {
        companyId: input.companyId,
    ...
    logInfo("[METRC] plant_batch_package_success", {
      companyId: input.companyId,
      plantBatchName,
      packageTag: input.packageTag,
      status: result.status,
    ...
    return {
      ok: true,
      status: result.status,
      message: "Plant batch package submitted to METRC sandbox.",
      endpoint: endpointKey,
      requestPayload,
      requestDebug,
      responsePayload: result.data,
