...
export class TaskService {
  ...
  async listRecent(companyId: string) {
    return this.repo.listRecent(companyId);
  }

  async deleteById(input: { companyId: string; actorUserId: string; role: string; taskLogId: string }) {
    ...
    if (changed.count === 0) throw new AppError("Task log not found", 404);
    await this.auditService.logAction({
      companyId: input.companyId,
      actorUserId: input.actorUserId,
  ...
  async deleteAll(input: { companyId: string; actorUserId: string; role: string }) {
    if (!["OWNER", "ADMIN"].includes(input.role)) {
