...
function deriveStrainAcronym(strain: string) {
...
type ChainWithPackages = { sourcePackages: { role: string; canonicalName: string }[] } | null;
...
export class OperationalWorkflowService {
  ...
  async createCultivation(input: {
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      const roles: Array<{ r: "A_GRADE_FLOWER" | "POPCORN" | "DRY_TRIM" | "FRESH_FROZEN"; name: string }> = [
  ...
  private sourcePackagesForDataHub(acronym: string, code: string, chain: ChainWithPackages) {
    if (chain) {
  ...
  async setFreshFrozenAllocation(input: { companyId: string; actorUserId: string; batchId: string; toExtractionGrams: number; extractionRunId?: string }) {
    const batch = await prisma.cultivationBatch.findFirst({ where: { companyId: input.companyId, id: input.batchId } });
    if (!batch) {
      throw new AppError("Cultivation batch not found", 404);
    }
  ...
  async finishCultivationPackaging(input: { companyId: string; actorUserId: string; runId: string }) {
    const run = await prisma.cultivationPackagingRun.findFirst({ where: { companyId: input.companyId, id: input.runId } });
    if (!run || run.status !== "IN_PROGRESS") {
  ...
  private async recomputeCultivationAutoStatus(companyId: string, batchId: string, actorUserId: string) {
  ...
  async createExtractionShell(input: { companyId: string; actorUserId: string; cultivationBatchId: string }) {
  ...
  private async requireExtractionRun(companyId: string, id: string, expected?: import("@prisma/client").ExtractionPhase) {
  ...
  async startBiomassPreparation(input: { companyId: string; actorUserId: string; runId: string }) {
  ...
  async packSocksStart(input: { companyId: string; actorUserId: string; runId: string }) {
  ...
  async packSocksStop(input: { companyId: string; actorUserId: string; runId: string }) {
    const run = await this.requireExtractionRun(input.companyId, input.runId, "BIOMASS_PREP_ACTIVE");
    if (!run.socksStartAt) {
  ...
  async sealProcessingInput(input: { companyId: string; actorUserId: string; runId: string; method: string; supplyUsed?: string }) {
    const run = await this.requireExtractionRun(input.companyId, input.runId, "READY_FOR_PROCESSING");
  ...
  async completeExtractionRun(input: { companyId: string; actorUserId: string; runId: string; outputGrams: number }) {
  ...
  async startExtractionPackaging(input: { companyId: string; actorUserId: string; extractionRunId: string; sku: string; gramsPerUnit: number; defaultTemplate?: string }) {
  ...
  async finishExtractionPackaging(input: { companyId: string; actorUserId: string; lotId: string }) {
    const lot = await prisma.packagingLot.findFirst({ where: { id: input.lotId, companyId: input.companyId } });
    if (!lot || lot.status !== "IN_PROGRESS") {
      throw new AppError("Packaging lot not in progress for company", 400);
    }
  ...
  laborCostPerPoundForOperationalLabor(input: { totalCost: number; hours: number; pounds: number }): { usdPerLb: number | null; hoursPerLb: number | null } {
  ...
  async deleteCultivationBatch(input: { companyId: string; actorUserId: string; batchId: string }) {
    const batch = await prisma.cultivationBatch.findFirst({ where: { companyId: input.companyId, id: input.batchId } });
    if (!batch) throw new AppError("Cultivation batch not found", 404);
  ...
  async listSourcePackages(input: { companyId: string; cultivationBatchId?: string }) {
  ...
  async createSourcePackage(input: {
    ...
    actorUserId: string;
    cultivationBatchId: string;
    role: "A_GRADE_FLOWER" | "POPCORN" | "DRY_TRIM" | "FRESH_FROZEN";
    ...
    if (!chain) throw new AppError("Source chain not found for cultivation batch", 404);
  ...
  async updateSourcePackage(input: {
    ...
    if (!current) throw new AppError("Source package not found", 404);
  ...
  async consumeSourcePackage(input: {
    ...
    if (!pack) throw new AppError("Source package not found", 404);
    if (pack.role === "DRY_TRIM") {
      ...
      if (!trim) throw new AppError("Trim flow state missing", 500);
      const total = g(trim.toExtractionGrams + trim.consumedGrams + input.grams);
      if (total - g(pack.sourceChain.cultivationBatch.trimGrams) > EPS) throw new AppError("Trim consumption exceeds harvested trim", 400);
      const row = await prisma.trimFlowState.update({
    ...
    if (pack.role === "FRESH_FROZEN") {
      ...
      if (!fresh) throw new AppError("Fresh frozen state missing", 500);
      const total = g(fresh.toExtractionGrams + input.grams);
      if (total - g(pack.sourceChain.cultivationBatch.freshFrozenGrams) > EPS) throw new AppError("Fresh frozen consumption exceeds harvested fresh frozen", 400);
      const row = await prisma.freshFrozenAllocation.update({
        where: { cultivationBatchId: pack.sourceChain.cultivationBatchId },
        data: { toExtractionGrams: { increment: g(input.grams) } }
      });
  ...
  async deleteSourcePackage(input: { companyId: string; actorUserId: string; sourcePackageId: string }) {
    const pack = await prisma.sourcePackage.findFirst({
      where: { id: input.sourcePackageId, sourceChain: { companyId: input.companyId } },
      include: { sourceChain: true }
    });
  ...
  async deleteExtractionRun(input: { companyId: string; actorUserId: string; runId: string }) {
    const run = await prisma.extractionRun.findFirst({ where: { id: input.runId, companyId: input.companyId } });
    if (!run) throw new AppError("Extraction run not found", 404);
    const lotCount = await prisma.packagingLot.count({ where: { extractionRunId: run.id } });
    if (lotCount > 0) throw new AppError("Cannot delete extraction run with packaging lots", 400);
  ...
  async deletePackagingLot(input: { companyId: string; actorUserId: string; lotId: string }) {
    const lot = await prisma.packagingLot.findFirst({ where: { id: input.lotId, companyId: input.companyId } });
    if (!lot) throw new AppError("Packaging lot not found", 404);
    if (lot.status === "COMPLETED") throw new AppError("Cannot delete completed packaging lot", 400);
