...
export class AdminService {
  ...
  async listUsers(input: { companyId: string }) {
  ...
  async updateUser(input: {
    ...
    if (changed.count === 0) throw new AppError("No user changes persisted", 400);
    const next = await this.repo.findUserById(input.companyId, input.targetUserId);
    if (!next) throw new AppError("User not found after update", 404);
    await this.auditService.logAction({
      companyId: input.companyId,
      actorUserId: input.actorUserId,
  ...
  async deleteUser(input: {
    ...
    if (changed.count === 0) throw new AppError("No user deleted", 404);
    await this.auditService.logAction({
      companyId: input.companyId,
      actorUserId: input.actorUserId,
