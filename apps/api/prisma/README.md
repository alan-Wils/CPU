...
| `migrations/` | **PostgreSQL only** (`migration_lock.toml`). `npm run prisma:migrate:deploy` in release/preDeploy (direct URL; see `scripts/prisma-migrate-deploy-production.mjs`). |
| NexBatch portal logins | After migrate on Postgres, run **`npm run prisma:seed:nexbatch`** (idempotent) to create `admin@nexbatch.com` / `owner@nexbatch.com` without re-running the full demo seed. |
