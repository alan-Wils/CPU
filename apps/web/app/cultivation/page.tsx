...
function hasManagerDeleteAccess() {
  ...
  const role = String(user?.role || "")
    ...
    .toUpperCase()
    .replace(/[\s-]+/g, "_");
  return role === "OWNER" || role === "ADMIN" || role === "MANAGER" || role === "OPERATIONS_MANAGER";
}
...
function hasCultivationWriteAccess() {
  ...
  return ["CULTIVATION", "CULTIVATION_SPECIALIST", "MANAGER", "OPERATIONS_MANAGER", "ADMIN", "OWNER"].includes(role);
}
...
export default function Cultivation() {
  ...
  function batchChainDbIdForLog(batchKey: string | undefined) {
    ...
    const id = String(batchKey).trim();
    const c = [...(s.cultivationBatches || []), ...(s.completedCultivationBatches || [])].find(
      (b: any) => b.id === id
    );
    if (c?.dbId) return String(c.dbId);
    const d = (s.dryFlowerBatches || []).find((b: any) => b.id === id);
    if (d?.source) {
      const c2 = [...(s.cultivationBatches || []), ...(s.completedCultivationBatches || [])].find(
        (b: any) => b.id === d.source
      );
      if (c2?.dbId) return String(c2.dbId);
    }
    return undefined;
  }
  ...
  function withLoggedBy(log: any) {
    ...
    const chainDb =
      String(log?.dbId || "").trim() || batchChainDbIdForLog(String(log?.batch || "")) || "";
    ...
    const finalLog = {
      ...
      data: {
        ...
        loggedAt,
        loggedAtIso,
        dbId: chainDb || (log.data && log.data.dbId) || undefined,
        cultivationDbId: chainDb || (log.data && (log.data as any).cultivationDbId) || undefined,
      },
    };
  ...
  function createCloneBatch() {
    ...
    const alreadyExists = s.cultivationBatches.some((b: any) => b.id === newBatch.id);
    if (!alreadyExists) {
      s.cultivationBatches.unshift(newBatch);
    } else {
      const existing = s.cultivationBatches.find((b: any) => b.id === newBatch.id);
      if (existing) Object.assign(existing, newBatch);
    }
  ...
  async function runDeleteBatch(batchId: string) {
    const victimBatch =
      ...
      null;
    const victimDbId = victimBatch?.dbId ? String(victimBatch.dbId) : "";
    ...
    const deletedLogCount = s.logs.filter(
      (log: any) =>
        ...
        (victimDbId &&
          (log.source === victimDbId ||
            log.batch === victimDbId ||
            log.linkedBatch === victimDbId))
    ).length;
    ...
    const deletedInBackend = await deleteRealCultivationBatchIfNeeded(batchId, wasCultivationBatch);
    if (!deletedInBackend) {
      showNotice(
        "Delete Failed",
        `Batch "${batchId}" could not be deleted from the backend, so nothing was removed locally.`,
        "Check that you are logged in as Admin or Owner, then try again. If the error persists, check the API terminal for details."
      );
      return;
    }

    s.cultivationBatches = (s.cultivationBatches || []).filter(
      (b: any) => b.id !== batchId && (!victimDbId || String(b?.dbId || "") !== victimDbId)
    );
    ...
    s.sourceBatches = (s.sourceBatches || []).filter((b: any) => {
      if (b.id === batchId) return false;
      const cid = String(b?.cultivationBatchId || b?.source || "");
      if (victimDbId && (cid === victimDbId || cid === batchId)) return false;
      return true;
    });
    ...
    s.logs = s.logs.filter((log: any) => {
      return (
        ...
        log.source !== batchId &&
        log.linkedBatch !== batchId &&
        !(victimDbId && (log.batch === victimDbId || log.source === victimDbId || log.linkedBatch === victimDbId))
      );
    });
  ...
  function deleteBatch(batchId: string) {
    ...
    showConfirm(
      ...
      "This permanently deletes the cultivation batch on the server, including linked source packages, extraction runs, extraction packaging lots, and cultivation-side packaging sessions. Local lists and related logs are cleared afterward."
    );
  }
  ...
  function logMatchesViewBatchForTasks(log: any, v: any) {
    if (!v) return false;
    const b = (x: any) => String(x || "").trim();
    const want = [b(v.id), b(v.dbId), b(v.source)].filter((x) => x.length > 0);
    const lb = b(log?.batch);
    const ls = b(log?.source);
    const lkb = b(log?.linkedBatch);
    for (const id of want) {
      if (id && (lb === id || ls === id || lkb === id)) return true;
    }
    if (b(v.dbId) && b(log?.data?.dbId) && b(v.dbId) === b(log?.data?.dbId)) return true;
    if (b(v.dbId) && lb === b(v.dbId)) return true;
    if (b(v.source) && lb === b(v.source)) return true;
    return false;
  }

  const selectedBatchLogs = viewBatch
    ? s.logs.filter((log: any) => logMatchesViewBatchForTasks(log, viewBatch))
    : [];
