...
export default function Extraction() {
  ...
  useEffect(() => {
    ...
    async function loadSharedCompanyData() {
      try {
        ...
        const sourceList = uniqueById(asArray(realSourceBatches));
        const extractionList = uniqueById(asArray(realExtractionBatches));
  ...
  async function runDeleteSourceBatch(batchId: string) {
    ...
    try {
      ...
      showNotice("Backend Delete Failed", "The source batch was removed locally, but the real database delete failed.");
    }
  ...
  async function runDeleteCompletedSourceBatch(batchId: string) {
    ...
    try {
      ...
      showNotice("Backend Delete Failed", "The completed source batch was removed locally, but the real database delete failed.");
    }
  ...
  async function create() {
    ...
    const usedSources = sourceInputs
      ...
      .map((row) => {
        ...
        return {
          ...
          cultivationBatchId: source?.cultivationBatchId || source?.dbId || source?.source || undefined,
    ...
    const batch = {
      ...
      cultivationBatchId: usedSources[0]?.cultivationBatchId || undefined,
      name: type,
      productType: type,
      sources: usedSources,
    ...
    const existing = (s.extractionBatches || []).find((b: any) => b.id === batch.id);
    if (existing) {
  ...
  async function runDeleteBatch(batchId: string) {
    ...
    try {
      ...
      showNotice("Backend Delete Failed", "The extraction batch was removed locally, but the real database delete failed.");
    }
  ...
  function deleteBatch(batchId: string) {
    ...
    showConfirm(
      ...
      "This removes the extraction batch and its matching packaging batch if one exists."
    );
  }
