...
import {
  downloadDataHubCsv,
  downloadDataHubFullCsv,
  downloadDataHubFullXlsx,
  downloadDataHubXlsx,
} from "@/lib/dataHubExport";
import Nav from "@/components/Nav";
import PageAccessGate from "@/components/PageAccessGate";
import { store } from "@/lib/store";
import { getAuthUser } from "@/lib/auth";
import { hasCompanyStoreChanged, loadBackendStore, saveBackendStore } from "@/lib/backendStore";
import {
  ...
  return Array.from(
    new Map(
      items
        ...
        .map((item: any) => [cleanId(item.id), item])
    ).values()
  );
}

function looksLikePrismaCuid(value: any) {
  const s = String(value || "").trim();
  return s.length >= 20 && /^c[a-z0-9]+$/i.test(s);
}

/** Backend cultivation row id (cuid) or display id — used to collapse duplicate chain roots. */
function cultivationChainDedupeKey(item: any) {
  const db = String(item?.dbId || "").trim();
  const id = String(item?.id || "").trim();
  const anchor = looksLikePrismaCuid(db) ? db : looksLikePrismaCuid(id) ? id : "";
  if (anchor) return anchor.toLowerCase();
  return cleanId(id);
}

function cultivationChainMergeScore(item: any) {
  return (
    (item?.dbId ? 8 : 0) +
    (/\.\d{2}-\d{4}/i.test(String(item?.id || "")) ? 4 : 0) +
    (Number(item?.plants) > 0 ? 1 : 0)
  );
}

function dedupeCultivationChainRoots(items: any[]) {
  const map = new Map<string, any>();
  for (const item of items || []) {
    if (!item || (!item.id && !item.dbId)) continue;
    const key = cultivationChainDedupeKey(item);
    ...
    map.set(
      key,
      cultivationChainMergeScore(item) > cultivationChainMergeScore(prev) ? item : prev
    );
  }
  return Array.from(map.values());
}

function cultivationMatchIds(cultivationBatch: any) {
  ...
  add(cultivationBatch?.id);
  add(cultivationBatch?.dbId);
  return ids;
}

function sourceTouchesCultivation(cultivationBatch: any, source: any) {
  const cultKeys = cultivationMatchIds(cultivationBatch);
  const sourceIds = [
    source?.sourceBatchId,
    source?.originalBatchId,
    source?.cultivationBatchId,
    source?.parentCultivationBatch,
    source?.source,
    source?.batchId,
  ]
    .map((v) => String(v || "").trim())
  ...
  if (sourceIds.some((sid) => cultKeys.has(sid) || cultKeys.has(cleanId(sid)))) return true;
  return sameBatchFamily(cultivationBatch, source);
}

function batchTouchesCultivationKeys(cultKeys: Set<string>, batch: any) {
  const ids = [
    batch?.source,
    batch?.sourceBatchId,
  ...
  if (ids.some((sid) => cultKeys.has(sid) || cultKeys.has(cleanId(sid)))) return true;
  return false;
}
...
function findRelatedSourceBatches(cultivationBatch: any, sourceBatches: any[]) {
  return sourceBatches.filter((source: any) => {
    return isSourceMaterialBatch(source) && sourceTouchesCultivation(cultivationBatch, source);
  });
}
...
function findRelatedFlowerOutputs(cultivationBatch: any, flowerBatches: any[]) {
  ...
  return flowerBatches.filter((batch: any) => {
    return (
      ...
      (batchTouchesCultivationKeys(cultKeys, batch) || sameBatchFamily(cultivationBatch, batch))
    );
  });
}
...
function findRelatedExtractionBatches(cultivationBatch: any, extractionBatches: any[], sourceBatches: any[]) {
  ...
  return extractionBatches.filter((ex: any) => {
    ...
    return (
      ...
      (batchTouchesCultivationKeys(cultKeys, ex) ||
        exSourceIds.some((id: string) => cultKeys.has(String(id)) || cultKeys.has(cleanId(id))) ||
        ...
        exSources.some(
          (src: any) =>
            cultKeys.has(String(src.id || "")) ||
            cultKeys.has(cleanId(src.id)) ||
            cultKeys.has(String(src.batchId || "")) ||
            cultKeys.has(cleanId(src.batchId)) ||
            cultKeys.has(String(src.sourceId || "")) ||
            cultKeys.has(cleanId(src.sourceId)) ||
...
function findRelatedPackagingBatches(cultivationBatch: any, extractionBatches: any[], packagingBatches: any[]) {
  ...
  return packagingBatches.filter((p: any) => {
    ...
    return (
      ...
      (batchTouchesCultivationKeys(cultKeys, p) ||
        extractionIds.includes(p.id) ||
        extractionIds.includes(p.sourceBatchId) ||
        extractionIds.includes(p.productionBatchId) ||
        pSourceIds.some((id: string) => cultKeys.has(String(id)) || cultKeys.has(cleanId(id))) ||
...
function canDeleteRecords() {
  ...
  return role === "MANAGER" || role === "OPERATIONS_MANAGER" || role === "ADMIN" || role === "OWNER";
}
...
export default function DataHub() {
  ...
  const [hubStage, setHubStage] = useState("all");
  const [hubStrain, setHubStrain] = useState("all");
  const [hubHideNoActivity, setHubHideNoActivity] = useState(false);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const changed = await hasCompanyStoreChanged();
      if (changed) {
        loadSharedData();
      }
    }, 1000);
  ...
  const cultivationBatches = dedupeCultivationChainRoots([
  ...
  const uniqueStages = useMemo(() => {
    ...
    for (const ch of chains) {
      const st = String(ch.cultivation?.stage || "").trim();
      if (st) set.add(st);
    }
  ...
  const uniqueStrains = useMemo(() => {
    const set = new Set<string>();
    for (const ch of chains) {
      const st = String(ch.cultivation?.strain || "").trim();
      if (st) set.add(st);
    }
    return Array.from(set).sort();
  }, [chains]);
  ...
  const filteredChains = useMemo(() => {
    return chains.filter((ch) => {
      const c = ch.cultivation;
      if (hubSearch.trim()) {
        const q = hubSearch.toLowerCase();
        const id = String(c?.id || "").toLowerCase();
        const str = String(c?.strain || "").toLowerCase();
        const dbs = String(c?.dbId || "").toLowerCase();
        if (!id.includes(q) && !str.includes(q) && !dbs.includes(q)) return false;
      }
      if (hubStage !== "all" && String(c?.stage || "") !== hubStage) return false;
      if (hubStrain !== "all" && String(c?.strain || "") !== hubStrain) return false;
      if (hubHideNoActivity) {
        const t =
          (ch.source?.length || 0) +
          (ch.flowerOutput?.length || 0) +
          (ch.extraction?.length || 0) +
          (ch.packaging?.length || 0);
        if (t === 0) return false;
      }
      return true;
    });
  }, [chains, hubSearch, hubStage, hubStrain, hubHideNoActivity]);
  ...
  const chainSummary = useMemo(() => {
    ...
    for (const ch of filteredChains) {
      sourceN += ch.source.length;
      flowerN += ch.flowerOutput.length;
      extN += ch.extraction.length;
      pkgN += ch.packaging.length;
      plants += num(ch.cultivation?.plants);
    }
    return { sourceN, flowerN, extN, pkgN, plants, rows: filteredChains.length };
  }, [filteredChains]);

  const filteredIdKey = useMemo(
    () => filteredChains.map((c) => String(c.id)).sort().join("|"),
    [filteredChains]
  );

  useEffect(() => {
    setExportSelectedIds(new Set(filteredChains.map((c) => String(c.id))));
  }, [filteredIdKey]);

  const selectedChainsForExport = useMemo(
    () => filteredChains.filter((c) => exportSelectedIds.has(String(c.id))),
    [filteredChains, exportSelectedIds]
  );
    ...
    <PageAccessGate allowedRoles={["VIEW_ONLY", "CULTIVATION", "EXTRACTION", "PACKAGING"]}>
      ...
      <div style={shellStyle}>
        ...
        <div style={cardStyle}>
          ...
          <p style={{ color: "#94a3b8", marginTop: 4, marginBottom: 16, lineHeight: 1.5 }}>
            Sortable-style grid: one row per cultivation chain. Use filters to narrow the list, then export the same rows to Excel or CSV.
          </p>
          ...
          >
            ...
            >
              <div>
                ...
                <p style={{ margin: "6px 0 0", color: "#94a3b8", fontSize: 13, maxWidth: 720 }}>
                  Use filters to narrow rows, then check the batches to include. Full export builds a workbook with sheets:
                  Summary, Cultivation, Source, Flower output, Extraction, and Packaging (all fields we have in the app).
                </p>
              </div>
              <div style={{ color: "#93c5fd", fontSize: 13, fontWeight: 700 }}>
                {selectedChainsForExport.length} selected / {filteredChains.length} shown
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
              <button
                ...
                onClick={() => {
                  if (selectedChainsForExport.length === 0) {
                    window.alert("Select at least one batch with the checkboxes in the table.");
                    return;
                  }
                  downloadDataHubFullXlsx(selectedChainsForExport, "data-hub-pipeline");
                }}
              >
                Export full pipeline (Excel)
              </button>
              <button
                type="button"
                ...
                onClick={() => {
                  ...
                  downloadDataHubFullCsv(selectedChainsForExport, "data-hub-pipeline");
                }}
              >
                Export full pipeline (CSV)
              </button>
              <button
                type="button"
                style={buttonStyle}
                onClick={() => {
                  ...
                  downloadDataHubXlsx(selectedChainsForExport, "data-hub-summary");
                }}
              >
                Summary only (Excel)
              </button>
              <button
                ...
                onClick={() => {
                  ...
                  downloadDataHubCsv(selectedChainsForExport, "data-hub-summary");
                }}
              >
                Summary only (CSV)
              </button>
            ...
            <div style={filterGridStyle}>
              <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ ...labelStyle, fontSize: 12 }}>Search batch or strain</span>
                <input
                  style={inputStyle}
                  placeholder="e.g. BLDR.26-0010 or Blue Dream"
                  value={hubSearch}
                  onChange={(e) => setHubSearch(e.target.value)}
                />
              </label>
              <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ ...labelStyle, fontSize: 12 }}>Stage</span>
                <select style={selectStyle} value={hubStage} onChange={(e) => setHubStage(e.target.value)}>
              ...
              <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ ...labelStyle, fontSize: 12 }}>Strain</span>
                <select style={selectStyle} value={hubStrain} onChange={(e) => setHubStrain(e.target.value)}>
                  <option value="all">All strains</option>
                  {uniqueStrains.map((st) => (
                    <option key={st} value={st}>
                      {st}
                    </option>
                  ))}
              ...
              >
                <input
                  type="checkbox"
                  checked={hubHideNoActivity}
                  onChange={(e) => setHubHideNoActivity(e.target.checked)}
                  style={{ width: 18, height: 18, cursor: "pointer" }}
                />
                <span style={{ color: "#cbd5e1", fontSize: 14 }}>Hide chains with no linked activity (all counts 0)</span>
              </label>
            </div>
          </div>
          ...
          {chains.length === 0 ? (
            ...
            <>
              ...
              >
                <div style={{ ...smallCardStyle, marginTop: 0, padding: 12 }}>
                  <div style={{ color: "#93c5fd", fontSize: 12, fontWeight: 700 }}>Rows shown</div>
                  <div style={{ fontSize: 22, fontWeight: 900 }}>
                    {chainSummary.rows}
                    <span style={{ color: "#64748b", fontSize: 14, fontWeight: 700 }}> / {chains.length}</span>
                  </div>
                </div>
                <div style={{ ...smallCardStyle, marginTop: 0, padding: 12 }}>
                  <div style={{ color: "#93c5fd", fontSize: 12, fontWeight: 700 }}>Total plants (filtered)</div>
                  <div style={{ fontSize: 22, fontWeight: 900 }}>{chainSummary.plants}</div>
                </div>
                <div style={{ ...smallCardStyle, marginTop: 0, padding: 12 }}>
                  <div style={{ color: "#93c5fd", fontSize: 12, fontWeight: 700 }}>Σ Source / Flower / Ext / Pkg</div>
                  <div style={{ fontSize: 18, fontWeight: 800, color: "#cbd5e1" }}>
                    {chainSummary.sourceN} · {chainSummary.flowerN} · {chainSummary.extN} · {chainSummary.pkgN}
                  </div>
                </div>
              </div>

              {filteredChains.length === 0 ? (
                <p style={mutedStyle}>No chains match the current filters.</p>
              ) : (
                  ...
                  <table style={tableStyle}>
                      ...
                      <tr>
                        ...
                        >
                          <input
                            ...
                            checked={
                              filteredChains.length > 0 &&
                              filteredChains.every((c) => exportSelectedIds.has(String(c.id)))
                            }
                            onChange={(e) => {
                              if (e.target.checked) {
                                setExportSelectedIds(new Set(filteredChains.map((c) => String(c.id))));
                        ...
                        <th style={thStyle}>Batch & strain</th>
                        <th style={thStyle}>Stage</th>
                        <th style={thStyle}>Status</th>
                        <th style={thStyle}>Room</th>
                        <th style={thStyle}>Bay</th>
                        <th style={{ ...thStyle, textAlign: "right" }}>Plants</th>
                        <th style={{ ...thStyle, textAlign: "right" }}>Src</th>
                        <th style={{ ...thStyle, textAlign: "right" }}>Flw</th>
                        <th style={{ ...thStyle, textAlign: "right" }}>Ext</th>
                        <th style={{ ...thStyle, textAlign: "right" }}>Pkg</th>
                    ...
                    <tbody>
                      {filteredChains.map((chain: any, idx: number) => (
                        <tr
                          key={chain.id}
                          style={{ background: idx % 2 === 0 ? "rgba(15, 23, 42, 0.35)" : "transparent" }}
                        >
                          <td
                            style={{
                              ...tdStyle,
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={exportSelectedIds.has(String(chain.id))}
                              onChange={() => {
                          ...
                          <td style={tdStyle}>
                            <div style={{ fontWeight: 800, color: "#f8fafc" }}>{show(chain.cultivation.id)}</div>
                            <div style={{ color: "#94a3b8", fontSize: 13, marginTop: 4 }}>{show(chain.cultivation.strain)}</div>
                          </td>
                          <td style={tdStyle}>{show(chain.cultivation.stage)}</td>
                          <td style={tdStyle}>{show(chain.cultivation.status)}</td>
                          <td style={tdStyle}>{show(chain.cultivation.flowerRoom || chain.cultivation.room)}</td>
                          <td style={tdStyle}>{show(chain.cultivation.flowerBay || chain.cultivation.bay)}</td>
                          <td style={tdNumStyle}>{show(chain.cultivation.plants)}</td>
                          <td style={tdNumStyle}>{chain.source.length}</td>
                          <td style={tdNumStyle}>{chain.flowerOutput.length}</td>
                          <td style={tdNumStyle}>{chain.extraction.length}</td>
                          <td style={tdNumStyle}>{chain.packaging.length}</td>
                          <td style={{ ...tdStyle, textAlign: "right", whiteSpace: "nowrap" }}>
                            <button type="button" style={{ ...greenButtonStyle, padding: "8px 12px" }} onClick={() => setSelectedChain(chain)}>
                              View
                            </button>
                            {canDeleteRecords() && (
                              <button
                                type="button"
                                style={{ ...deleteButtonStyle, padding: "8px 12px", marginLeft: 6 }}
