...
function isPackageSetRecord(batch: any) {
  const id = String(batch?.id || "").toUpperCase();
  return lower(batch?.sourceArea) === "packaging" || id.includes("-PKG-");
}
...
function hasPackagingWriteAccess() {
  ...
  return role !== "VIEW_ONLY" && ["PACKAGING", "PACKAGING_SPECIALIST", "MANAGER", "OPERATIONS_MANAGER", "ADMIN", "OWNER"].includes(role);
}
...
export default function Packaging() {
  ...
  useEffect(() => {
    const role = String(getAuthUser()?.role || "").toUpperCase();
    setCanDeleteRecords(role === "OWNER" || role === "ADMIN" || role === "MANAGER" || role === "OPERATIONS_MANAGER");
    ...
    async function loadPackagingData() {
      try {
        ...
        const nextReady = s.packagingBatches.filter(isExtractionPackagingBatch).filter((b: any) => getRemainingGrams(b) > 0);
        const nextInProgress = s.inProgressPackagingBatches.filter(isExtractionPackagingBatch);

        setSelected((current: any) => {
          if (current?.id) {
            const still = nextReady.find((b: any) => b.id === current.id);
            if (still) return still;
          }
          return nextReady[0] || null;
        });

        setSelectedInProgress((current: any) => {
          if (current?.id) {
            const still = nextInProgress.find((b: any) => b.id === current.id);
            if (still) return still;
          }
          return nextInProgress[0] || null;
        });
        ...
        try {
          ...
          if (active) {
            const nextReady = (s.packagingBatches || []).filter(isExtractionPackagingBatch).filter((b: any) => getRemainingGrams(b) > 0);
            const nextInProgress = (s.inProgressPackagingBatches || []).filter(isExtractionPackagingBatch);
            setSelected(nextReady[0] || null);
            setSelectedInProgress(nextInProgress[0] || null);
            setRefresh((n) => n + 1);
          }
        } catch (backupError) {
