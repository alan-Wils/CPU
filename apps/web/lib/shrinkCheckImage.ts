...
/** Downscale very large photos before upload / OCR. */
export async function shrinkCheckImageFileIfLarge(file: File, maxEdge = 2000): Promise<File> {
  ...
  try {
    ...
    const blob = await new Promise<Blob | null>((r) => canvas.toBlob((b) => r(b), "image/jpeg", 0.88));
    if (!blob) return file;
    const base = file.name.replace(/\.[^.\\/]+$/, "") || "check";
